...
In the tsx code here, you implemented the `resolveDataHash()` and `resolveTxHash()` resolvers. You create a function `getTxHash()` that builds a basic transaction to send lovelace to your address with a custom datum. `resolveDataHash()` is used to convert the datum to a hash, and `resolveTxHash()` is used to convert the unsigned transaction to a hash. you also used the `useState()` React hook to store the data hash and transaction hash.
...
In the tsx code here, you implemented the `resolveSlotNo()` and `resolveEpochNo()` resolvers. You used `setInterval()` React method to update the current slot and epoch number every second for the `preprod` network.

You can now run `npm run dev` and go to http://localhost:3000 to see the result. After loading the application, you should be able to slot and epoch number updating every second displayed on the page. Notice that a Cardano epoch is 432,000 slots long which is about 5 days, so only the change in slot number should be noticeable.
